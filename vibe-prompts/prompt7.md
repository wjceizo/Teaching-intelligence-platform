...
1.教师上传题目在初始代码下面可以添加一个完整代码（教师可以选择填写与不填写），以供教师测试运行。在教师提供测试用例后，可以一键根据完整代码生成期望输出（无完整代码则无法生成）。
2.学生运行完得出结果，若有隐藏用例，则不需要每个都告诉学生是否通过，只需要在测试结果显示隐藏用例通过个数。
3.教师在新建题目时，课程栏选择只能选择自己教授的课程。
4.教师可以删除实训的题目。
5.每个实训的分数由教师直接自定义（默认100分），每个测试用例由总分平均，不需要为每个实训单独算分。
6.在学生全屏显示的时候，背景是透明的，会显示出原本的下一层内容，请修改。
7.在教师的代码实训，管理题目旁的新建题目也应该和管理题目一样用框框起来。
8.学生在代码实训中只应该查看到自己上的课的实训题目。